...
|G的子群 |陪集坐标| 使用阶段|
|--|--|--|
|所有角的方向都为0 |角方向坐标范围 0..2186 | 阶段1|
|所有边的方向都为0|边方向坐标范围 0..2047|阶段1|
|UD中间层四个边都在中间层|UDSlice 坐标范围0..493|阶段1
|边方向为0且UD-slice的四条边都在中间层|FlipUDSlice 坐标范围 0..494*2048 - 1|阶段1|
|所有角都在对应位置，可以角的方向可以不为0|范围 0..40319|阶段2|
|U、D两层的边都在对应位置，边可以翻转|阶段2边的位置范围 0..40319|阶段2|
|UDSlice的四条边都在对应位置，可以有翻转|排序后的坐标范围 0..11879|阶段2|

上面有些坐标会因为冗余而被提出，后面章节会做说明。
...
说明右陪集`C0*g`中有相同的角方向坐标，也就是所有的元素的角方向是相同的。如果两个状态的角方向坐标相同，就认为在同一个右陪集内（证明略）。所以角方向坐标和右陪集C0是一一对应的。
...
(PS:使用的是IDA*算法，IDA*的核心是启发函数,魔方还原算法的启发函数就是给出最少还原步数)

我们在初始化的时候其实已知最小的还原步数,保存每个状态对应的最小还原步数。这样可以动态对搜索路径进行裁剪，加快搜索速度。

|裁剪表|对称坐标|原始坐标|表长度|最大裁剪深度|
|--|--|--|--|--|
|阶段1|FlipUDSlice(64430等效类别)|UDTwist(2187情况)|140,908,410 |12|
|阶段2|角排列(2768等效类别)|边的排列(40320情况) |111,605,760| 18|
